...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast('success', t('auth.accountCreated'), t('auth.accountCreatedSuccess'));
      setLocation('/login');
    } catch {
      showToast('error', t('auth.registerError'), t('auth.emailInUse'));
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center bg-primary p-4 sm:p-6 md:p-8 min-h-screen safe-area-inset-bottom">
        <div className="w-full max-w-md">
          ...
          <div className="mb-6 sm:mb-8">
            ...
            <p className="text-sm sm:text-base text-white/80">{t('auth.startOrganizing')}</p>
          </div>
